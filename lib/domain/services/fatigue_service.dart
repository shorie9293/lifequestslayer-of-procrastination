...
class FatigueService {
  ...
  /// 疲労レベル (0: 快調, 1: やや疲れ, 2: 疲労, 3: 限界)
  static int fatigueLevel(Player player) {
    final p = progress(player);
    if (p >= 1.0) return 3;
    if (p >= 0.7) return 2;
    if (p >= 0.4) return 1;
    return 0;
  ...
  static double progress(Player player) {
    return (player.dailyTasksCompleted / severeThreshold(player))
        .clamp(0.0, 1.0);
  }
