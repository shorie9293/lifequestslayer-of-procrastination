...
class GameViewModel extends ChangeNotifier with WidgetsBindingObserver {
  ...
  int get dailyMissionProgress =>
      _player.dailyTasksCompleted.clamp(0, dailyMissionGoal);
  bool get isDailyMissionComplete =>
      _player.dailyTasksCompleted >= dailyMissionGoal;
  int get weeklyMissionProgress =>
      _player.weeklySRankCompleted.clamp(0, weeklyMissionGoal);
  bool get isWeeklyMissionComplete =>
      _player.weeklySRankCompleted >= weeklyMissionGoal;

  // --- ストリーク ---
  int get streakDays => _player.streakDays;

  // --- 見積もり時間 ---
  /// 今日の戦場（activeTasks）の見積もり時間合計（分）
  int get dailyEstimatedMinutes {
    int sum = 0;
    for (final t in activeTasks) {
      if (t.targetTimeMinutes != null) sum += t.targetTimeMinutes!;
    }
    return sum;
  }

  /// ギルド未着手（guildTasks）の見積もり時間合計（分）
  int get guildEstimatedMinutes {
    int sum = 0;
    for (final t in guildTasks) {
      if (t.targetTimeMinutes != null) sum += t.targetTimeMinutes!;
    }
    return sum;
  }
  ...
  // --- 称号進捗 ---
  List<({TitleDefinition def, int progress, bool isUnlocked})>
      get titleProgressList {
    return TitleService.getTitleProgressList(_player);
  }

  List<Task> get recurringTasks =>
      _tasks.where((t) => t.repeatInterval != RepeatInterval.none).toList();

  List<Task> get guildTasks => _tasks
  ...
  bool _isVisibleForPlayer(Task t) {
    if (_player.canUseSkill(Job.cleric) &&
        t.repeatInterval != RepeatInterval.none) {
  ...
  final _adventurerTheme = ThemeData(
    ...
    appBarTheme:
        const AppBarTheme(backgroundColor: Color(0xFF3e3b16), elevation: 0),
  ...
  final _warriorTheme = ThemeData(
    ...
    appBarTheme:
        const AppBarTheme(backgroundColor: Color(0xFF3e1616), elevation: 0),
    colorScheme: ColorScheme.fromSwatch(
        brightness: Brightness.dark,
        primarySwatch: Colors.red,
  ...
  final _clericTheme = ThemeData(
    ...
    appBarTheme:
        const AppBarTheme(backgroundColor: Color(0xFF163e3e), elevation: 0),
    colorScheme: ColorScheme.fromSwatch(
        brightness: Brightness.dark,
        primarySwatch: Colors.cyan,
  ...
  final _wizardTheme = ThemeData(
    ...
    appBarTheme:
        const AppBarTheme(backgroundColor: Color(0xFF16213e), elevation: 0),
    colorScheme: ColorScheme.fromSwatch(
        brightness: Brightness.dark,
        primarySwatch: Colors.deepPurple,
  ...
  void editTask(String taskId, String title,
      {QuestRank rank = QuestRank.B,
      RepeatInterval repeatInterval = RepeatInterval.none,
      List<int>? repeatWeekdays,
      List<SubTask>? subTasks,
  ...
  String? acceptTask(String taskId) {
    ...
    final currentRankActiveCount =
        activeTasks.where((t) => t.rank == task.rank).length;
  ...
  Map<String, dynamic>? completeTask(String taskId) {
    ...
    if (_player.dailyTasksCompleted >=
        FatigueService.severeThreshold(_player)) {
      bonusMessages.add("🌙 今日の英雄は十分戦った。宿屋で休んで明日に備えよ！");
    } else if (_player.dailyTasksCompleted >=
        FatigueService.warnThreshold(_player)) {
  ...
  Future<void> loadData() async {
    ...
    try {
      ...
      debugPrint('[VM] player load error: $e\n$s');
      _loadFailed = true;
    }
    try {
      _tasks = await _taskRepository.loadTasks();
    } catch (e, s) {
      debugPrint('[VM] tasks load error: $e\n$s');
      _tasks = [];
      _loadFailed = true;
    }
    ...
    try {
      ...
      debugPrint('[VM] tutorialStep load error: $e\n$s');
    }
    try {
      _hasSeenConcept = await _settingsRepository.getHasSeenConcept();
    } catch (e, s) {
      debugPrint('[VM] hasSeenConcept load error: $e\n$s');
    }
    try {
      _fontSizeScale = await _settingsRepository.getFontSizeScale();
    } catch (e, s) {
      debugPrint('[VM] fontSizeScale load error: $e\n$s');
    }
    ...
    try {
      _knowledgeQuestEnabled =
          await _settingsRepository.getKnowledgeQuestEnabled();
    } catch (e, s) {
      debugPrint('[VM] knowledgeQuest load error: $e\n$s');
    }

    try {
      _tutorialSkipped = await _settingsRepository.getTutorialSkipped();
    } catch (e, s) {
      debugPrint('[VM] tutorialSkipped load error: $e\n$s');
    }
    try {
      _tutorialChoiceMade = await _settingsRepository.getTutorialChoiceMade();
    } catch (e, s) {
      debugPrint('[VM] tutorialChoiceMade load error: $e\n$s');
    }

    try {
      final fatiguePopupDate = await _settingsRepository.getFatiguePopupDate();
      if (fatiguePopupDate != null &&
          DateUtils.isSameDay(fatiguePopupDate, DateTime.now())) {
        _hasShownFatiguePopupToday = true;
      }
    } catch (e, s) {
      debugPrint('[VM] fatiguePopup load error: $e\n$s');
    }
    ...
    try {
      ...
      debugPrint('[VM] checkAndResetMissions error: $e\n$s');
    }
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.paused ||
      ...
      saveData().catchError(
          (e) => debugPrint('GameViewModel: lifecycle save failed: $e'));
    }
  }
