...
class _GemShopScreenState extends State<GemShopScreen> {
    ...
    return Column(
      ...
      children: [
        ...
        if (!iap.available)
          ...
          ...iap.products
              .map((product) => _buildProductCard(context, iap, product)),
      ],
    );
  }

  Widget _buildProductCard(
      BuildContext context, IAPService iap, ProductDetails product) {
    ...
    return Card(
      ...
      child: ListTile(
        leading:
            const Icon(Icons.diamond, color: Colors.purpleAccent, size: 32),
        title: Row(
          children: [
        ...
        subtitle: bonus.isNotEmpty
            ? Text(bonus, style: const TextStyle(color: Colors.amberAccent))
            : null,
        trailing: SemanticHelper.interactive(
          testId: SemanticHelper.createTestId(
              SemanticTypes.button, 'buy_${product.id}'),
          label: '$labelを購入',
  ...
  void _confirmPurchase(BuildContext context, IAPService iap,
      ProductDetails product, String label) {
    showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          SemanticHelper.interactive(
            testId: SemanticHelper.createTestId(
                SemanticTypes.button, 'cancel_gem_purchase'),
            label: '購入をキャンセル',
            child: TextButton(
          ...
          SemanticHelper.interactive(
            testId: SemanticHelper.createTestId(
                SemanticTypes.button, 'confirm_gem_purchase'),
            label: '購入する',
            child: ElevatedButton(
            style:
  ...
  Widget _buildUseCard({
    ...
    return Card(
      ...
      child: ListTile(
        ...
        trailing: SemanticHelper.interactive(
          testId: SemanticHelper.createTestId(
              SemanticTypes.button, 'use_gems_${title.hashCode}'),
          label: title,
              ...
              ? () => showDialog(
                    ...
                    builder: (ctx) => AlertDialog(
                      ...
                      actions: [
                        SemanticHelper.interactive(
                          testId: SemanticHelper.createTestId(
                              SemanticTypes.button, 'cancel_gem_use'),
                          label: '使用をキャンセル',
                          child: TextButton(
                        ...
                        SemanticHelper.interactive(
                          testId: SemanticHelper.createTestId(
                              SemanticTypes.button, 'confirm_gem_use'),
                          label: '宝石を使う',
                          child: ElevatedButton(
                          style: ElevatedButton.styleFrom(
                              backgroundColor: Colors.purple[700]),
  ...
  void _showResult(
      BuildContext context, bool ok, String successMsg, String failMsg) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
