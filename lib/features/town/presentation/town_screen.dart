...
class TownScreen extends StatelessWidget {
  ...
  Map<String, dynamic> _getHomeData(player) {
    if (player.homeItems.contains("home_4")) {
      return {'name': 'ギルドハウス', 'image': 'assets/images/home_4.png'};
    }
    if (player.homeItems.contains("home_3")) {
      return {'name': '石造りの家', 'image': 'assets/images/home_3.png'};
    }
    if (player.homeItems.contains("home_2")) {
      return {'name': '木の小屋', 'image': 'assets/images/home_2.png'};
    }
    if (player.homeItems.contains("home_1")) {
      return {'name': '小さなテント', 'image': 'assets/images/home_1.png'};
    }
                      ...
                      _buildShopSection(context,
                          ...
                          items: [
                            _ShopItem(
                                ...
                                name: "小さなテント",
                                price: 1000,
                                description: "とりあえず雨風はしのげる。"),
                            _ShopItem(
                                id: "home_2",
                                name: "木の小屋",
                                price: 5000,
                                description: "ちゃんとした壁がある拠点。"),
                            _ShopItem(
                                id: "home_3",
                                name: "石造りの家",
                                price: 20000,
                                description: "魔物にも壊されない頑丈な家。"),
                            _ShopItem(
                                id: "home_4",
                                name: "ギルドハウス",
                                price: 100000,
                                description: "立派なギルドの拠点。機能美が光る。"),
                          ],
                          player: player,
                          viewModel: viewModel),
                      const SizedBox(height: 24),
                      _buildSkinSection(context,
                          ...
                          items: [
                            _ShopItem(
                                ...
                                name: "旅人のマント",
                                price: 500,
                                description: "長旅に耐える丈夫なマント。"),
                            _ShopItem(
                                id: "skin_2",
                                name: "戦士の勲章",
                                price: 3000,
                                description: "歴戦の戦士だけが身につける勲章。"),
                            _ShopItem(
                                id: "skin_3",
                                name: "大魔導士の杖",
                                price: 15000,
                                description: "魔力を底上げする伝説の杖のレプリカ。"),
                            _ShopItem(
                                id: "skin_4",
                                name: "王冠",
                                price: 500000,
                                description: "王者が被るにふさわしい冠。"),
                          ],
                          player: player,
                          viewModel: viewModel),
                    ],
          ...
          return Card(
            ...
            child: ListTile(
              ...
              trailing: isOwned
                  ...
                  : SemanticHelper.interactive(
                      testId: SemanticHelper.createTestId(
                          SemanticTypes.button, 'buy_${item.id}'),
                      label: '${item.name}を${item.price}金貨で購入',
                      child: ElevatedButton.icon(
                      icon: const Icon(Icons.shopping_cart, size: 16),
                      label: Text("${item.price}"),
                      style: ElevatedButton.styleFrom(
                        backgroundColor: player.coins >= item.price
                            ? Colors.amber[700]
                            : Colors.grey,
                        foregroundColor: Colors.white,
                      ),
              ...
              trailing: SemanticHelper.interactive(
                ...
                onPressed: () {
                  ...
                  ScaffoldMessenger.of(context)
                      .showSnackBar(const SnackBar(content: Text("スキンを外した")));
                },
              ),
              ),
            ),
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          SemanticHelper.interactive(
            testId: SemanticHelper.createTestId(
                SemanticTypes.button, 'cancel_buy'),
            label: '購入をやめる',
            child: TextButton(
          ...
          SemanticHelper.interactive(
            testId: SemanticHelper.createTestId(
                SemanticTypes.button, 'confirm_buy'),
            label: '購入する',
            child: ElevatedButton(
  ...
  // --- 宿屋システム（新機能：疲労回復） ---
  Widget _buildInnSection(
      BuildContext context, player, GameViewModel viewModel) {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 12),
        _buildInnItem(
            context, viewModel, player, 0, "裏路地のテント", 50, "翌日の疲労限界+2(微回復)"),
        _buildInnItem(
            context, viewModel, player, 1, "ふかふかのベッド", 200, "翌日の疲労限界+5(中回復)"),
        _buildInnItem(context, viewModel, player, 2, "王様のスイートルーム", 1000,
            "翌日の疲労限界+12(完全回復)"),
      ],
    );
  }

  Widget _buildInnItem(BuildContext context, GameViewModel viewModel, player,
      int type, String name, int price, String desc) {
    return Card(
      color: Colors.black54,
      child: ListTile(
        ...
        trailing: SemanticHelper.interactive(
          ...
          onPressed: () {
            if (player.coins >= price) {
              ...
              ScaffoldMessenger.of(context)
                  .showSnackBar(const SnackBar(content: Text("金貨が足りないぜ")));
            }
          },
        ),
        ),
  ...
  void _restAtInn(BuildContext context, GameViewModel viewModel, int type,
          ...
          SemanticHelper.interactive(
            ...
            onPressed: () {
              ...
              if (err != null) {
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(content: Text("ぐっすり休んだ！明日はもっと頑張れそうだ！")));
              }
            },
        ...
        Card(
          ...
          child: ListTile(
            ...
            trailing: SemanticHelper.interactive(
              ...
              onPressed: () =>
                  _showTitleSelectDialog(context, viewModel, player),
            ),
            ),
          ),
        ),
  ...
  Widget _buildTitleProgressCard({
    ...
    return Card(
      ...
      shape: RoundedRectangleBorder(
        ...
        side: BorderSide(
          color:
              isUnlocked ? Colors.amber.withValues(alpha: 0.6) : Colors.white12,
          width: 1,
        ),
      ),
      child: Padding(
              ...
              children: [
                ...
                if (isEquipped)
                  ...
                  SemanticHelper.interactive(
                    ...
                    onPressed: onEquip,
                    child:
                        const Text("装備", style: TextStyle(color: Colors.amber)),
                  ),
                  ),
              ],
            ),
