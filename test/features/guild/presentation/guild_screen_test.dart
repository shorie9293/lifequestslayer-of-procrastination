...
void main() {
  group('GuildScreen 見積もり時間表示テスト', () {
    ...
    testWidgets(
        'ギルドクエストに targetTimeMinutes がある場合、'
      ...
      await tester.runAsync(() async {
        ...
        // 見積もり時間付きのクエストを追加（受注しない → guildTasks に留まる）
    ...
    testWidgets('複数ギルドクエストの見積もりが合計表示される', (tester) async {
      late ({TaskViewModel task, PlayerViewModel player, SettingsViewModel settings}) vms;
    ...
    testWidgets(
        'ギルドクエストの一部だけ targetTimeMinutes がある場合、'
    ...
    testWidgets(
        'ギルドクエストの targetTimeMinutes が null の場合、見積もり表示は出ない',
        (tester) async {
      late ({TaskViewModel task, PlayerViewModel player, SettingsViewModel settings}) vms;
    ...
    testWidgets('クエストがない場合は見積もり表示が出ない', (tester) async {
      late ({TaskViewModel task, PlayerViewModel player, SettingsViewModel settings}) vms;
  ...
  group('GuildScreen 緊急依頼セクション', () {
    testWidgets('緊急クエストがない時は緊急セクションが表示されない',
        (tester) async {
      late ({TaskViewModel task, PlayerViewModel player, SettingsViewModel settings}) vms;
      ...
      // 緊急クエストがないのでセクションは非表示
      expect(find.byKey(AppKeys.guildUrgentSection), findsNothing);
    });

    testWidgets('24時間以内の期限があるクエストで緊急セクションが表示される',
        (tester) async {
      late ({TaskViewModel task, PlayerViewModel player, SettingsViewModel settings}) vms;
    ...
    testWidgets('緊急クエストがなく通常クエストのみの場合でも通常表示は崩れない',
        (tester) async {
      late ({TaskViewModel task, PlayerViewModel player, SettingsViewModel settings}) vms;
